...
```{python graf_tiempos_matrices_py}
GenerarGraficos(df_cifrado_cesar_py).barras('integrante', 'Tiempo', 'Tiempos Cifrado Cesar')
```
...
Ahora en Python. Se crea un objeto de la clase TrabajoDataframes con la ruta de Base salarios el cual se utilizará para próximos ejercicios.
...
```{python obj_regresion_logsitica_py}
...
GenerarGraficos(df_regresion_logistica_py).barras('integrante', 'Tiempo', 'Tiempos Regresion Logistica')
```
### Modelo estocástico
