...
```{python objeto TrabajoDataframes Muertes}
obj_Muertes = TrabajoDataframes('data/Muertes_costa_rica.xlsx')
```

```{python medir limpieza}
obj_Muertes.medir_tiempo("limpiar_datos()")
```

Se crea un objeto de la clase GenerarDataframes el cual se utilizara para los proximos ejercicios

```{python objeto GenerarDataframes}
obj_generadordf = GenerarDataframes(['Alejandro', 'Eyeri', 'Santiago', 'Paula'])
```

```{python limpieza tiempos}
...
```{python objeto TrabajoDataframes Muertes limpias}
...
```{python medir tiempo graficos paralelo}
obj_Muertes.medir_tiempo("generar_graficos()")
```

```{python tiempos graficos paralelo}
df_limpieza = ob_gen_limpieza.tiempos_simples([9.71, 8.28, 8.36, 17.07])
```

```{python grafico graficos en paralelo}
...
Se crea un objeto de la clase TrabajoDataframes con la ruta de Base salarios el cual se utilizara para proximos ejercicios
