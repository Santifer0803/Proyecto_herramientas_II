...
muertes.original <- read_excel("data/Muertes_costa_rica.xlsx")
...
#' @param b Escalar que representa el sesgo.
#' @param X Matriz de características de entrada.
#' @param y Vector de etiquetas objetivo.
#' @param num.iterations Número de iteraciones para el descenso del gradiente.
#' @param learning.rate Tasa de aprendizaje para la actualización de los parámetros.
#' 
...
#' utilizando los datos de entrenamiento proporcionados. Luego, realiza predicciones 
#' sobre los datos de testeo y calcula la precisión del modelo tanto en entrenamiento 
#' como en testeo.
#'
#' @param X.train Matriz de características de entrenamiento.
#' @param y.train Vector de etiquetas de entrenamiento.
#' @param X.val Matriz de características de testeo.
#' @param y.val Vector de etiquetas de testeo.
#' @param num.iterations Número de iteraciones para la optimización. Por defecto es 2000.
#' @param learning.rate Tasa de aprendizaje para la optimización. Por defecto es 0.5.
#'
...
#'   }
regresion.logistica <- function(X.train, y.train,
                                X.val, y.val,
                                num.iterations = 2000,
                                learning.rate = 0.5) {
...
```{r tiempos_reg_log}
tiempos.regresion.log <- data.frame(
  integrante = c("Eyeri", "Santiago", "Paula", "Alejandro"), 
  tiempo = c(0.08, 0.06, 0, 0)
  )
```
...
```{r grafico_reg_log}
ggplot(tiempos.regresion.log, aes(x = integrante, y = tiempo)) +
  geom_col(fill = "deepskyblue2") +
  labs(
    x = "Integrante",
    y = "Tiempo (segundos)",
    title = "Tiempos de la regresión logística",
    caption = "Fuente: elaboración propia"
  ) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
prob.muerteh <- sapply(1:45, function(i) sapply(((20 + i)):116, function(k) qx.hombres[[k, 5 + k]]))
...
# Cantidad de primas
idh <- sapply(1:filmat, function(m) which(!matriz.rndh[m, ])[1])

# Anualidades
anh <- sapply(1:filmat, function(n) sum((1 + j) ^ -(0:(min(idh[n], (65 - 19 - n)) - 1))))

# Rango de años de cada persona desde que llega a 65 años hasta que fallece
annosh.65 <- sapply(20:64, function(o) c((65 - o):(idh[[o - 19]])))

# Beneficios para trabajadores y pensionados
benh <- sapply(1:length(annosh.65), function(p) (ifelse(idh + c(20:64) < 65, 5000000 * ((1 + j) ^ -(idh)), sum(300000 * 13 * (1 + j) ^ -annosh.65[[p]]) + 1000000 * ((1 + j) ^ -(idh))))[p])
...
#'
modelo.estocastico <- function(matriz.prob, prom.primas, lista.primas){
  ...
  id <- sapply(1:filmat, function(m) which(!matriz.rnd[m, ])[1])
  
  # Anualidad
  an <- sapply(1:filmat, function(n) sum((1 + j) ^ -(0:(min(id[n], (65 - 19 - n)) - 1))))
  
  # Rango de años de cada persona desde que llega a 65 años hasta que fallece
  annos.65 <- sapply(20:64, function(o) c((65 - o):(id[[o - 19]])))
  
  # Beneficios para trabajadores y pensionados
  ben <- sapply(1:length(annos.65), function(p) (ifelse(id + c(20:64) < 65, 5000000 * ((1 + j) ^ -(id)), sum(300000 * 13 * (1 + j) ^ -annos.65[[p]]) + 1000000 * ((1 + j) ^ -(id))))[p])
...
ini <- Sys.time()
primas.estocasticash <- modelo.estocastico(matriz.probh, 0, lista.primash)
fin <- Sys.time()
...
tiempos.estocast <- data.frame(
  integrante = c("Eyeri", "Santiago", "Paula", "Alejandro"), 
  tiempo = c(0.032, 0.046, 0, 0)
  )
```

Gráfico del modelo estocástico.

```{r graf_modelo_estocastico}
ggplot(tiempos.estocast, aes(x = integrante, y = tiempo)) +
  geom_col(fill = "pink2") +
  labs(
    x = "Integrante",
    y = "Tiempo (segundos)",
    title = "Tiempos del modelo estocástico",
    caption = "Fuente: elaboración propia"
  ) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r tiempo_cumprod}
microbenchmark(producto.acumulado = {
  prod.acum.cumprod(runif(100000, min = .Machine$double.eps, max = 2))
},
times = 10,
unit = "seconds")
```
...
```{r tiempo_sapply}
microbenchmark(prod.acum.sapply(runif(
  100000, min = .Machine$double.eps, max = 2
)), times = 10, unit = "seconds")
```

```{r tiempos_producto_acumulado}
tiempos.producto.acumulado <- data.frame(
  integrante = c("Santiago", "Paula", "Eyeri", "Alejandro"),
  cumprod = c(0.001, 0.01, 0.006, 0),
  sapply = c(27.93, 44.54, 27.82, 0)
)
```

```{r graf_tiempos_prod_acum}
# Convertir el formato de ancho a largo
producto.acum.largo <-
  gather(tiempos.producto.acumulado,
         key = "condicion",
         value = "tiempo",
         -integrante)

# Se diferencian las barras
producto.acum.largo$condicion <-
  factor(
    producto.acum.largo$condicion,
...
ggplot(producto.acum.largo, aes(x = integrante, y = tiempo, fill = condicion)) +
  geom_col(position = "dodge") +
  labs(
    x = "Integrante",
    y = "Tiempo (segundos)",
    title = "Tiempos del producto acumulado",
    caption = "Fuente: elaboración propia"
  ) +
  scale_fill_manual(values = c(
    "cumprod" = "seagreen3",
...
#' 
#' @param texto Texto que se desea cifrar.
#' @param desplazamiento Número entero que indica el desplazamiento en el alfabeto.
#' 
...
```{r prueba_chartr}
microbenchmark(cifrado.chartr(principito, 5), times = 10, unit = "seconds")
```
...
#'
#' @param texto Texto que se desea cifrar.
#' @param desplazamiento Número entero que indica cuántas posiciones desplazar
...
```{r prueba_stringi}
microbenchmark(cifrado.stringi(principito, 5), times = 10, unit = "seconds")
```

```{r tiempos_cifrado_cesar}
tiempos.cifrado.cesar <- data.frame(
  integrante = c("Santiago","Paula", "Eyeri", "Alejandro"),
  chartr = c(0.012, 0.02, 0.016, 0),
  stringi = c(1.25, 2.46	, 1.3, 0)
)
```

```{r graf_tiempos_textos}
# Convertir el formato de ancho a largo
texto.largo <-
  gather(tiempos.cifrado.cesar,
         key = "condicion",
         value = "tiempo",
         -integrante)

# Se diferencian las barras
texto.largo$condicion <-
  factor(
    texto.largo$condicion,
...
ggplot(texto.largo, aes(x = integrante, y = tiempo, fill = condicion)) +
  geom_col(position = "dodge") +
  labs(
    x = "Integrante",
    y = "Tiempo (segundos)",
    title = "Tiempos del cifrado Cesar",
    caption = "Fuente: elaboración propia"
  ) +
  scale_fill_manual(values = c(
    "chartr" = "aquamarine3",
