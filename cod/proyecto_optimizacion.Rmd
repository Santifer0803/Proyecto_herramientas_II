...
tiempos.limpieza <- data.frame(
  ...
  con.datatable = c(3.05496),
  sin.datatable = c(4.77649)
)
```
...
tiempos.graficos <- data.frame(
  ...
  histogramas = c(8.46395, 4.83059),
  barras = c(13.31152, 8.06775)
)
```
...
tiempos.matrices <- data.frame(
  integrante = c("Eyeri", "Santiago"), 
  tiempo = c(5.068372, 5.99183)
  )
```
...
tiempos.eliminar.columnas <- data.frame(
  integrante = c("Eyeri", "Santiago"), 
  tiempo = c((741.48) / 1000, (766.26) / 1000000))
```
