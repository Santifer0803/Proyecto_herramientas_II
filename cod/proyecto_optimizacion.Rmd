...
```{r descarga_datos}
empleados <- read_excel("data/Base_empleados.xlsx")
...
Los tiempos del modelo estocástico.

```{r tiempos_reg_log}
tiempos.estocast <- data.frame(
  integrante = c("Eyeri", "Santiago", "Paula", "Alejandro"), 
  tiempo = c(0, 0.046, 0, 0)
  )
```
...
```{r producto acumulado usando cumprod}
acumulado.cumprod <- function(vector) {
...
```{r prueba cumprod}
microbenchmark(acumulado.cumprod(runif(
  100000, min = .Machine$double.eps, max = 2
)), times = 10, unit = "seconds")
```

```{r producto acumulado usando sapply}
acumulado.sapply <- function(vector) {
...
```{r prueba sapply}
microbenchmark(acumulado.sapply(runif(
  100000, min = .Machine$double.eps, max = 2
)), times = 10, unit = "seconds")
```

```{r tiempos producto acumulado}

tiempos.producto.acumulado <- data.frame(
  integrante = c("Santiago","Paula", "Eyeri", "Alejandro"),
  cumprod = c(0, 0.00997996, 0, 0),
  sapply = c(0, 44.53777	, 0, 0)
)
...
```{r cargar archivo.txt para el cifrado cesar}
principito <- read_file("el_principito.txt")
```

```{r cifrado cesar usando chartr}
...
```{r cifrado cesar usando stringi}
...
```{r tiempos cifrado cesar}
tiempos.cifrado.cesar <- data.frame(
  integrante = c("Santiago","Paula", "Eyeri", "Alejandro"),
  chartr = c(0, 0.02062909, 0, 0),
  stringi = c(0, 2.459928	, 0, 0)
)
```
