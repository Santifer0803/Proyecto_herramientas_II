...
tiempos.limpieza <- data.frame(
  integrante = c("Santiago","Paula"),
  con.datatable = c(3.05496, 7.309475),
  sin.datatable = c(4.77649, 11.26839)
)
```
...
tiempos.graficos <- data.frame(
  ...
  histogramas = c(8.46395, 4.83059, 8.531366),
  barras = c(13.31152, 8.06775, 14.893497)
)
```
...
tiempos.mediam <- data.frame(
  integrante = c("Santiago", "Eyeri", "Paula"),
  media_movil = c((3.15827) / 1000, (6.7584) / 1000, (10.75752) / 1000)
)
```
...
tiempos.matrices <- data.frame(
  ...
  tiempo = c(5.068372, 5.99183, 7.257987)
  )
```
...
tiempos.eliminar.columnas <- data.frame(
  integrante = c("Eyeri", "Santiago", "Paula"), 
  tiempo = c((741.48) / 1000, (766.26) / 1000000), (3.56987	) / 1000 )
```
